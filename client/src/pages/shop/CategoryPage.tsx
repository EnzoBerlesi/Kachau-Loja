...
// Mapeamento de slugs de URL para IDs de categoria
...
function CategoryPage() {
  ...
  // Título e ícone da categoria baseados no slug da URL
  const categoryInfo = slug && CATEGORY_MAP[slug] 
    ? CATEGORY_MAP[slug] 
    : { name: 'Produtos', icon: '🛒' };
  ...
  return (
    <>
      ...
      <div className="w-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 pt-12 pb-8 min-h-screen">
        <div className="container mx-auto px-4 pt-8">          <div className="flex items-center mb-8">
            <span className="text-3xl mr-3">{categoryInfo.icon}</span>
            <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">
              {categoryInfo.name}
            </h1>
            {activeCategory && (
              <span className="ml-4 text-sm text-gray-400">({products.length} produtos)</span>
            )}
          </div>

...
// Mock de dados (substitua pelo seu banco depois)
...
export default function Notebooks() {
  ...
  const notebooksFiltrados = mockNotebooks.filter(notebook => {
    // Filtro por preço
    if (notebook.preco < precoRange[0] || notebook.preco > precoRange[1]) return false;
    ...
    // Filtro por marca
    if (marcasSelecionadas.length > 0 && !marcasSelecionadas.includes(notebook.marca)) return false;
    
    // Filtro por busca
    if (termoBusca && !notebook.nome.toLowerCase().includes(termoBusca.toLowerCase())) return false;
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-purple-900 to-gray-900 text-white">
      ...
      <div className="pt-28 pb-16 px-4 text-center">
        ...
        <p className="text-gray-300 max-w-2xl mx-auto">
          Os melhores notebooks com os melhores preços. Frete grátis para todo Brasil!
        </p>
      </div>
      ...
      {showFilters && (
        <div className="bg-gray-800/90 border-b border-gray-700">
          <div className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-4 gap-6">
            {/* Faixa de Preço */}
            <div>
              <h3 className="font-medium mb-3 text-pink-400">Faixa de Preço</h3>
              <div className="space-y-4">
                <input
                  ...
                  step="100"
                  value={precoRange[1]}
                  onChange={(e) => setPrecoRange([precoRange[0], Number(e.target.value)])}
                ...
                {marcasDisponiveis.map(marca => (
                  <label key={marca} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => setMarcasSelecionadas(prev => 
                        prev.includes(marca) 
            ...
            {/* Avaliação */}
            <div>
              <h3 className="font-medium mb-3 text-pink-400">Avaliação Mínima</h3>
              <div className="flex items-center gap-2">
                {[1, 2, 3, 4, 5].map(star => (
                  <button
                    key={star}
                    onClick={() => setAvaliacaoMinima(star)}
                    className={`p-1 ${avaliacaoMinima >= star ? 'text-yellow-400' : 'text-gray-500'}`}
                  >
                    <Star size={20} fill={avaliacaoMinima >= star ? 'currentColor' : 'none'} />
                  </button>
                ))}
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div key={notebook.id} className="group bg-gray-800/50 hover:bg-gray-800/70 border border-gray-700/50 hover:border-pink-500/30 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-pink-500/10">
              ...
              <div className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-lg line-clamp-2">{notebook.nome}</h3>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm text-gray-400">({notebook.numeroAvaliacoes})</span>
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="text-xl font-bold text-pink-400">
                      R$ {notebook.preco.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </p>
                    {notebook.precoOriginal && (
                      <p className="text-xs text-gray-400 line-through">
                        R$ {notebook.precoOriginal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </p>
                    )}
                  </div>
        ...
        {notebooksFiltrados.length === 0 && (
          <div className="text-center py-16">
            <p className="text-xl text-gray-400">Nenhum notebook encontrado com esses filtros</p>
            <button 
              onClick={() => {

...
function Carrinho() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6 font-sans">
      <h1 className="text-3xl font-bold text-purple-400 mb-6">Seu Carrinho</h1>
      ...
      {carrinho.length === 0 ? (
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg max-w-4xl mx-auto">
            ...
            {carrinho.map((item) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <img src={item.imagem} alt={item.nome} className="w-16 h-16 rounded" />
                  <div>
          ...
          <div className="mt-6 border-t border-gray-700 pt-4 text-right text-lg">
            <p className="text-gray-300">Subtotal: R$ {subtotal.toFixed(2)}</p>
            {desconto > 0 && (
              <p className="text-green-400">Desconto: -{(desconto * 100).toFixed(0)}%</p>
            )}
            <p className="font-bold text-purple-400 text-xl">Total: R$ {total.toFixed(2)}</p>
          </div>
